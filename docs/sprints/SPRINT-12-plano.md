...
**Status:** Em andamento — S12-01 concluída (ver `docs/sprints/SPRINT-12-investigacao.md`), S12-02 a seguir
...
| S12-02 | **Separar "análise" de "modelagem" como duas chamadas distintas ao Gemini.** Novo serviço `bpmn_analysis.py`: recebe a transcrição, devolve um JSON estruturado (schema Pydantic) com atores **classificados interno/externo** (achado de S12-01: a IA já consegue listar os atores certos, só não usa essa informação na hora de montar o XML — pedir a classificação explicitamente na análise deve ajudar o passo de modelagem a não esquecer), atividades com tipo, pontos de decisão com tipo de gateway, pools/raias sugeridas, regras de negócio — sem gerar XML. `bpmn_generator.py` passa a receber esse JSON já estruturado, com uma responsabilidade só: traduzir a análise em XML BPMN válido. Cada etapa loga em `record_usage` com seu próprio `stage` (`"analysis"`, `"modeling"`, em vez do `"bpmn_generation"` único de hoje) — o painel "Uso de IA" já mostra o breakdown por etapa sem mudança nenhuma nele, porque a agregação já é por `stage`. | 5 |
| S12-03 | **Reforçar `bpmn_validator.py` com checagem estrutural para pool/raia ausente em ator externo — prioridade alta, confirmada por S12-01.** Se a análise (S12-02) classificar um ator como externo e o XML não tiver um `bpmn:participant` correspondente, reprovar e disparar o retry corretivo (mesmo padrão do fix do gateway combinado, S10, `2e7f53f`) — regra de prompt sozinha já existe e já está sendo ignorada, então só validação estrutural resolve. Adicionar também a checagem de nuance encontrada em S12-01: gateway de divisão (`parallelGateway`/`inclusiveGateway`) com menos de 2 saídas, ou de junção com menos de 2 entradas. | 5 |
