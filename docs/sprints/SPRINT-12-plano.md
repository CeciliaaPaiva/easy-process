...
**Status:** Concluída — ver `docs/releases/RELEASE-S12.md` e `docs/sprints/SPRINT-12-investigacao.md`
...
| S12-04 | **Concluída.** Adicionado 1 exemplo concreto (ERRADO/CERTO) ao prompt de `bpmn_analysis.py`, contrastando um gateway split com 1 saída só (o bug real achado em "pedido pelo app") com a versão correta de 2+ saídas — mesmo padrão pra join. **Validado contra a API real**: rodando o mesmo caso de novo, os 5 gateways gerados já saíram todos corretos na primeira tentativa (3 splits com 2 saídas, 2 joins com 2 entradas), sem precisar do retry de S12-03 — reduz custo por processo em vez de só empurrar o problema pra validação corrigir depois. | 1 |
| S12-05 | **Concluída.** `extract_usage` agora loga `logger.warning` quando `model_name` não está em `_PRICING_PER_MILLION`, em vez de cair em custo $0 silencioso. 2 testes novos (`test_unknown_model_logs_warning`, `test_known_model_does_not_log_warning`). `CLAUDE.md` ganhou seção "Atualizando a tabela de preços do Gemini" documentando quando atualizar (novo `GEMINI_MODEL_*` configurado, ou aviso de mudança de preço do Google) e o link da tabela oficial. | 1 |
